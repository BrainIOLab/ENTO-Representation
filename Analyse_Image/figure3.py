...
for _ in range(repeats):
    ...
    labels_neuron=[i.split('_')[2] for i in temp]
    ...
    neuron_accuracy.append(accuracy_score(labels_neuron, y_pred) ) # 准确率
...
plt.savefig(os.path.join(r'E:\Image_paper\Project\plot', 'figure3', 'figure3_2.eps'), dpi=600, format='eps')
...
# —— 组装数值与误差（列表→均值±STD；标量→误差0）——
names = list(dict_accuracy.keys())
vals_raw = [dict_accuracy[k] for k in names]
...
for v in vals_raw:
    if hasattr(v, '__len__') and not isinstance(v, (str, bytes)):
        ...
        if v.size > 1:
            stds.append(float(np.nanstd(v, ddof=1)))   # ← 使用 STD
        else:
            stds.append(0.0)
    else:
...
ax.set_ylabel('Decoding accuracy (mean ± SD)',  # ← 文案也改为 SD
              fontsize=fontsize_, fontname=fontname_, fontweight=fontweight_)
ax.set_ylim(0, 1.02)
...
plt.savefig(os.path.join(r'E:\Image_paper\Project\plot', 'figure3', 'figure3_1.eps'), dpi=600, format='eps')